    ...
    public class DataIntegrator
    {
        ...
        public void Run()
        {
            ...
            try
            {
                ...
                StartAppServiceRun();
                _logMsg = String.Format("Starting Run with _Run.AppServiceRunID = {0} ", _Run.AppServiceRunID);
                logger.Info(_logMsg);
                _ctxSVC.SvcIns_AppServiceRunLog(specp.Domain.Entities.Com.Service.TENCIA, _Run.AppServiceRunID, null, "INFO", _logMsg, DateTime.Now, specp.Domain.Entities.Com.SysUsers.SERVICE, ref _outError);
                //logger.Trace("1.2	Create log file for Data Exchange Session/Run with < DXCHGRID>");

                _logMsg = "Process Destination/Tencia Backlog";
                logger.Info(_logMsg);
                _ctxSVC.SvcIns_AppServiceRunLog(specp.Domain.Entities.Com.Service.TENCIA, _Run.AppServiceRunID, null, "INFO", _logMsg, DateTime.Now, specp.Domain.Entities.Com.SysUsers.SERVICE, ref _outError);
                ProcessUploadedBacklog();

                _logMsg = "Extract Transform and Stage (ETS)";
                logger.Info(_logMsg);
                _ctxSVC.SvcIns_AppServiceRunLog(specp.Domain.Entities.Com.Service.TENCIA, _Run.AppServiceRunID, null, "INFO", _logMsg, DateTime.Now, specp.Domain.Entities.Com.SysUsers.SERVICE, ref _outError);

                ETS();

                _logMsg = "Upload Staged Data";
                logger.Info(_logMsg);
                _ctxSVC.SvcIns_AppServiceRunLog(specp.Domain.Entities.Com.Service.TENCIA, _Run.AppServiceRunID, null, "INFO", _logMsg, DateTime.Now, specp.Domain.Entities.Com.SysUsers.SERVICE, ref _outError);

                UploadStagedData();

                _logMsg = "Notify Problems";
                logger.Info(_logMsg);
                _ctxSVC.SvcIns_AppServiceRunLog(specp.Domain.Entities.Com.Service.TENCIA, _Run.AppServiceRunID, null, "INFO", _logMsg, DateTime.Now, specp.Domain.Entities.Com.SysUsers.SERVICE, ref _outError);
                ...
                _logMsg = String.Format("Finished Run with _AppServiceRunId = {0} ", _Run.AppServiceRunID);
                logger.Info(_logMsg);
                _ctxSVC.SvcIns_AppServiceRunLog(specp.Domain.Entities.Com.Service.TENCIA, _Run.AppServiceRunID, null, "INFO", _logMsg, DateTime.Now, specp.Domain.Entities.Com.SysUsers.SERVICE, ref _outError);
            ...
            {
                _ctxSVC.SvcIns_AppServiceRunLog(specp.Domain.Entities.Com.Service.TENCIA, _Run.AppServiceRunID, null, "FATAL", e.Message, DateTime.Now, specp.Domain.Entities.Com.SysUsers.SERVICE, ref _outError);
        ...
        {
            logger.Trace("Process Destination/TENCIA Backlog ");
            ...
            // Get backlog pending items in queue
            GetAppServiceQueue(specp.Domain.Entities.Com.State.SENT_PENDING, -1, null);
            ProcessUploadedBacklogCurrentQueue("PENDING");
            // Get backlog last sent items in queue
            GetAppServiceQueue(specp.Domain.Entities.Com.State.STAGED_SENT, -1, null);
            ProcessUploadedBacklogCurrentQueue("UPLOADED");
        }

        void ProcessUploadedBacklogCurrentQueue(string queName)
        {
            _logMsg = String.Format("Total items {0} in queue = {1}",queName, _Queue.Count());
            logger.Info(_logMsg);
            _ctxSVC.SvcIns_AppServiceRunLog(specp.Domain.Entities.Com.Service.TENCIA, _Run.AppServiceRunID, null, "INFO", _logMsg, DateTime.Now, specp.Domain.Entities.Com.SysUsers.SERVICE, ref _outError);
            ...
            foreach (var qi in _Queue)  //100-TOBESENT
            {
                ...
                var DestinationLocation = FindFileAtDestination(qi.FileName);
                switch (DestinationLocation)
                {
                    case "UPLOADED":
                        ...
                        UpdateAppServiceQueueStatus(DestinationLocation, qi.AppServiceQueueID, qi.FileName, specp.Domain.Entities.Com.State.SENT_PENDING, qi.RetryAttempts + 1, qi.StatusID, qi.StatusID);
                        break;
                    case "SUCCESS":
                        ...
                        UpdateAppServiceQueueStatus(DestinationLocation, qi.AppServiceQueueID, qi.FileName, specp.Domain.Entities.Com.State.SENT_SUCCESS, 0, specp.Domain.Entities.Com.Status.CLOSED, specp.Domain.Entities.Com.Status.CLOSED);
                        break;
                    case "REJECTED":
                        ...
                        UpdateAppServiceQueueStatus(DestinationLocation, qi.AppServiceQueueID, qi.FileName, specp.Domain.Entities.Com.State.SENT_REJECTED, 0, specp.Domain.Entities.Com.Status.CLOSED, specp.Domain.Entities.Com.Status.CLOSED);
                        break;
                    default: // MISSING
                        ...
                        UpdateAppServiceQueueStatus(DestinationLocation, qi.AppServiceQueueID, qi.FileName, specp.Domain.Entities.Com.State.SENT_MISSING, 0, specp.Domain.Entities.Com.Status.CLOSED, specp.Domain.Entities.Com.Status.CLOSED);
                        break;
                }
            }
        }
        ...
        void ETS()
        {
            ...
            foreach (var qi in _Queue)  //100-TOBESENT
            {
                ...
                try
                {
                    ...
                    UpdateAppServiceQueueStatus("STAGED", qi.AppServiceQueueID, file, specp.Domain.Entities.Com.State.TO_BE_SENT_STAGED, qi.RetryAttempts, qi.StatusID, specp.Domain.Entities.Com.Status.CLOSED);
                }
                catch (Exception e)
                {
        ...
        //            UpdateAppServiceQueueStatus("SENT", qi.AppServiceQueueID, Path.GetFileName(f), specp.Domain.Entities.Com.State.STAGED_SENT, 0, qi.StatusID, qi.StatusID);
        ...
        //            UpdateAppServiceQueueStatus("PENDING", qi.AppServiceQueueID, Path.GetFileName(f), specp.Domain.Entities.Com.State.STAGED_PENDING, qi.RetryAttempts + 1, qi.StatusID, qi.StatusID);
        ...
        {
            logger.Trace("Upload Staged Data");
            //1.	For each log entry in <TransferData> Log with status in (STAGED, STAGED-PENDING)
            //1.1	Upload/FTP file with .tmp extension to <designated folder at destination/tencia>
            //1.1.1	Rename file to .txt extension at remote 
            //1.2	When Failure
            ...
            // Get staged pending items in queue
            GetAppServiceQueue(specp.Domain.Entities.Com.State.STAGED_PENDING, -1, null);
            UploadStageDataCurrentQueue("STAGED_PENDING");
            // Get  last stage items in queue
            GetAppServiceQueue(specp.Domain.Entities.Com.State.TO_BE_SENT_STAGED, -1, null);
            UploadStageDataCurrentQueue("TO_BE_SENT_STAGED");

        }

        void UploadStageDataCurrentQueue(string queName)
        {
            ...
            {
                logger.Info("Uploading " +qi.FileName);
                
                var f = Path.Combine(ETLUtil._TraxDILocalStagePath,qi.FileName);
               
                if (!File.Exists(f))
                {
                    //logger.Error(qi.FileName + " not found in queue");

                    _logMsg = f + " not found in queue";
                    logger.Info(_logMsg);
                    _ctxSVC.SvcIns_AppServiceRunLog(specp.Domain.Entities.Com.Service.TENCIA, _Run.AppServiceRunID, null, "INFO", _logMsg, DateTime.Now, specp.Domain.Entities.Com.SysUsers.SERVICE, ref _outError);
                    continue;
                }
                ...
                {
                    // move to archive
                    File.Move(f, Path.Combine(ETLUtil._TraxDILocalArchivePath, qi.FileName));

                    //UpdateAppServiceQueue(qi.AppServiceQueueID,qi.FileName,specp.Domain.Entities.Com.State.STAGED_SENT , 0, qi.StatusID);
                    // updated state of queue item in this run
                    //AddAppServiceRunQueue(qi.AppServiceQueueID, specp.Domain.Entities.Com.State.STAGED_SENT, 0, qi.StatusID);

                    UpdateAppServiceQueueStatus("STAGED_SENT", qi.AppServiceQueueID, qi.FileName, specp.Domain.Entities.Com.State.STAGED_SENT, 0, qi.StatusID, qi.StatusID);


                    logger.Info("Archived file {0}",qi.FileName);
                }
                else
                {
                    //UpdateAppServiceQueue(qi.AppServiceQueueID, qi.FileName, specp.Domain.Entities.Com.State.STAGED_PENDING, qi.RetryAttempts+1, qi.StatusID);
                    // updated state of queue item in this run
                    //AddAppServiceRunQueue(qi.AppServiceQueueID, specp.Domain.Entities.Com.State.STAGED_PENDING, qi.RetryAttempts+1, qi.StatusID);
                    UpdateAppServiceQueueStatus("STAGED_PENDING", qi.AppServiceQueueID, Path.GetFileName(f), specp.Domain.Entities.Com.State.STAGED_PENDING, qi.RetryAttempts + 1, qi.StatusID, qi.StatusID);
        ...
        void UpdateAppServiceQueueStatus(string Message, int AppServiceQueueID, string FileName, int StateID, int RetryAttempts, int StatusID, int RunQueueStatusID)
        {
            UpdateAppServiceQueue(AppServiceQueueID, FileName, StateID, RetryAttempts, StatusID);
            AddAppServiceRunQueue(AppServiceQueueID, StateID, RetryAttempts, RunQueueStatusID);

            _logMsg = String.Format("Message={0} AppServiceQueueID={1} FileName={2} StateID={3} RetryAttempt={4} StatusID={5} RunQueueStatusID {6}", Message, AppServiceQueueID, FileName, StateID, RetryAttempts, StatusID, RunQueueStatusID);
            logger.Info(_logMsg);
            _ctxSVC.SvcIns_AppServiceRunLog(specp.Domain.Entities.Com.Service.TENCIA, _Run.AppServiceRunID, AppServiceQueueID, "INFO", _logMsg, DateTime.Now, specp.Domain.Entities.Com.SysUsers.SERVICE, ref _outError);
